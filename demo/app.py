...
dm_model = '/opt/dynet/NeurboParser/model/\
            dm.adadelta.lstm200.layer2.h100.drop0.25.model'

pas_pruner = '/opt/dynet/NeurboParser/model/english_pas.pruner.model'
pas_model = '/opt/dynet/NeurboParser/model/\
             pas.adadelta.lstm200.layer2.h100.drop0.25.model'

psd_pruner = '/opt/dynet/NeurboParser/model/english_pas.pruner.model'
psd_model = '/opt/dynet/NeurboParser/model/\
             psd.adadelta.lstm200.layer2.h100.drop0.25.model'
...
def hello():
    ...
    return "Welcome to the pyNeurboParser Demo Web App!!<br><br>\
            ...
            Please note that the demo service takes \
            about 5 seconds to process text and return a response.\
            "
...
def dm(text):
    ...
    return semantic_parse(text, parser_file, dm_pruner,
                          data_file, dm_model, prediction)
...
def psd(text):
    ...
    # show the user profile for that user
    return semantic_parse(text, parser_file, psd_pruner,
                          data_file, psd_model, prediction)
...
def pas(text):
    ...
    # show the user profile for that user
    return semantic_parse(text, parser_file, pas_pruner,
