...
def train(parser_file, train_epochs, pruner_epochs, file_train, file_dev,
		  ...
		  lemma_dim, word_dim, pos_dim, lstm_dim, mlp_dim, num_lstm_layers,
		  use_word_dropout, word_dropout_rate, trainer, srl_train_cost_false_positives,
		  srl_train_cost_false_negatives, file_model, file_prediction, log_file):
...
if __name__ == "__main__":
	...
	file_pretrained_embedding = os.path.join(args.neurbo_path,'embedding/glove.100.pruned')
	form_path = os.path.join(args.neurbo_path,'semeval2015_data/%s/data'%args.form)
	file_train = os.path.join(form_path,'english/english_%s_augmented_train.sdp'%args.form)
	file_dev = os.path.join(form_path, 'english/english_%s_augmented_dev.sdp'%args.form)
	file_test = os.path.join(form_path, 'english/english_id_%s_augmented_test.sdp'%args.form)
	file_model= os.path.join(args.model,'%s.%s.lstm%s.layer%s.h%s.drop%s.model' % (
		args.form,
		trainer,
		lstm_dim, 
		num_lstm_layers, 
	...
	file_prediction=os.path.join(args.pred,'/%s.%s.lstm%s.layer%s.h%s.drop%s.pred' %(
		args.form,
		trainer,
		lstm_dim,
		num_lstm_layers,
	...
	train(parser_file, train_epochs, pruner_epochs, file_train, file_dev,
		  file_pruner_model, use_pretrained_embedding, file_pretrained_embedding,
		  lemma_dim, word_dim, pos_dim, lstm_dim, mlp_dim, num_lstm_layers,
		  use_word_dropout, word_dropout_rate, trainer, srl_train_cost_false_positives,
		  srl_train_cost_false_negatives, file_model, file_prediction, log_file)
